/**
 * Scrapes Flipkart search results for a query — Puppeteer fallback path.
 * See that file's doc comment for the overall rationale (this replaces the
 * tab the extension used to open per-site). This is now only reached when
 * the fast Cheerio-based fetch path (`flipkart.js`) fails to find results,
 * e.g. if Flipkart starts serving a bot-check page or changes its markup
 * in a way the plain-HTML parser can't handle.
 */
async function scrapeFlipkartViaPuppeteer(browser, query) {
...
module.exports = { scrapeFlipkartViaPuppeteer };
